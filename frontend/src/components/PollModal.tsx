...
interface PollModalProps {
  visible: boolean;
  poll: Poll | null;
  onSubmit: (pollId: string, selectedOption: number) => Promise<{ data?: { counts?: number[] } }>;
  onDismiss: () => void;
}

const OPTION_LETTERS = ['A', 'B', 'C', 'D'];

const PollModal: React.FC<PollModalProps> = ({ visible, poll, onSubmit, onDismiss }) => {
  const [selected, setSelected] = useState<number | null>(null);
  const [submitting, setSubmitting] = useState<boolean>(false);
  const [submitted, setSubmitted] = useState<boolean>(false);
  const [resultCounts, setResultCounts] = useState<number[] | null>(null);
      ...
      <View style={styles.overlay}>
        <View style={styles.sheet}>
          ...
          {poll.options.map((opt, i) => {
            const pct =
              showResults && total > 0
                ? Math.round(((displayCounts?.[i] ?? 0) / total) * 100)
                : 0;
            const isSelected = selected === i || poll.answered === i;
            const maxCount = displayCounts ? Math.max(...displayCounts) : 0;
            const isWinner = showResults && displayCounts != null && displayCounts[i] === maxCount;
                ...
                {showResults && (
                  <View
                    style={[
                      ...
                      {
                        width: `${pct}%`,
                        backgroundColor: isCorrect ? '#DCFCE7' : isWinner ? '#7C3AED20' : '#F3F4F6',
                      },
                    ]}
                  />
                )}
                ...
                <View style={styles.optionInner}>
                  ...
                  >
                    <Text
                      style={[
                        styles.letterText,
                        (isSelected && !showResults) || (showResults && (isWinner || isCorrect))
                  ...
                  <Text
                    style={[
                      styles.optText,
                      showResults && isWinner && { fontWeight: '700' as const, color: '#1E1B4B' },
                      isCorrect && { color: '#15803D', fontWeight: '700' as const },
                    ]}
                    numberOfLines={2}
                  >
                    {opt}{isCorrect ? ' ✓' : ''}
                  ...
                  {showResults && (
                    <Text
                      style={[
                        styles.pct,
                        isWinner && { color: '#7C3AED', fontWeight: '700' as const },
          ...
          {showResults ? (
            <View style={styles.footer}>
              <Text style={styles.totalText}>
                {total} student{total !== 1 ? 's' : ''} responded
              </Text>
...
const styles = StyleSheet.create({
  ...
  pill:        { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FEF2F2', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 20, gap: 6 },
  ...
  question:    { fontSize: 16, fontWeight: '700', color: '#111827', marginBottom: 18, lineHeight: 22 },
  option:      { borderRadius: 10, borderWidth: 1.5, borderColor: '#E5E7EB', marginBottom: 10, overflow: 'hidden', backgroundColor: '#FAFAFA' },
  optionSelected: { borderColor: '#7C3AED', backgroundColor: '#FAF5FF' },
  optionResult:   { borderColor: '#E5E7EB', backgroundColor: '#fff' },
  optionWinner:   { borderColor: '#7C3AED' },
  optionCorrect:  { borderColor: '#16A34A' },
  bar:         { position: 'absolute', top: 0, left: 0, bottom: 0, borderRadius: 8 },
  optionInner: { flexDirection: 'row', alignItems: 'center', padding: 12, gap: 10 },
  letter:      { width: 28, height: 28, borderRadius: 14, backgroundColor: '#F3F4F6', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  ...
  submitBtn:   { marginTop: 8, backgroundColor: '#7C3AED', borderRadius: 12, paddingVertical: 14, alignItems: 'center' },
  ...
  doneBtn:     { backgroundColor: '#7C3AED', borderRadius: 10, paddingVertical: 10, paddingHorizontal: 24 },
